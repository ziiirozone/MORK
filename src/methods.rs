    ...
    pub fn add_known_part(
        ...
        for k in 0..y0.len() {
            for N in 0..y0[k].len() {
                for N1 in 1..=N {
                    y[j][k][N] +=
                        self.secondary_weights[N][N1][j] * self.h_powers[N1] * y0[k][N - N1]
                }
                sum = 0.;
                for &j1 in J {
    ...
    fn approximate(
        ...
        for task in self.queue.iter() {
            match task {
                Task::Explicit(j) => {
                    let j = *j;
                    self.add_known_part(&(0..self.s).collect(), j, &mut y, y0, &F);
                    if j != self.s {
    ...
    pub fn add_known_part(
        ...
        for k in 0..y0.len() {
            for N in 0..y0[k].len() {
                if self.nodes[j] != 0. {
                    for N1 in 1..=N {
                        y[j][k][N] += self.coefficients[N1][j] * self.h_powers[N1] * y0[k][N - N1]
                    }
                }
                sum = 0.;
                for &j1 in J {
